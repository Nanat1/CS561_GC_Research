...
for gc_level in "${GC_LEVELS[@]}"; do
    echo ""
    echo "--- Threshold: $FINISH_THRESHOLD%, GC Start Level: $gc_level% ---"
    ...
    $SSH "sudo nvme zns reset-zone -a /dev/$ZBD && \
        ...
        sudo ./plugin/zenfs/util/zenfs mkfs \
            ...
            --enable_gc=true \
            --gc_start_level=$gc_level \
            --force 2>/dev/null"

    # Phase 1: fillrandom warmup (output discarded)
    ...
    # Clear log so only overwrite phase is recorded
    echo "  Clearing FEMU log before overwrite..."
    > $FEMU_LOG

    # Phase 2: overwrite (measured)
    ...
    $SSH "cd ~/rocksdb && sudo ./db_bench \
        ...
        --value_size=$VALUE_SIZE \
        --key_size=$KEY_SIZE \
        --compression_type=none > overwrite_${NUM_OPS}_${gc_level}.log 2>&1"

    OVERWRITE_OUTPUT=$($SSH "cat ~/rocksdb/overwrite_${NUM_OPS}_${gc_level}.log")

    # Parse pages_to_write from FEMU log
    FINISH_STATS=$(grep -a "\[FINISH\] zone:" $FEMU_LOG \
        | tail -n +2 \
        | awk -F'pages_to_write: ' '{sum+=$2; count++} END {if (count>0) print sum, count, sum/count; else print "0 0 N/A"}')

    TOTAL_PAGES=$(echo $FINISH_STATS | awk '{print $1}')
    FINISH_COUNT=$(echo $FINISH_STATS | awk '{print $2}')
    LATENCY=$(echo "$OVERWRITE_OUTPUT" | grep "^overwrite" | awk '{print $3}')
    THROUGHPUT=$(echo "$OVERWRITE_OUTPUT" | grep "^overwrite" | awk '{print $(NF-1)}')
    TIMESTAMP=$(date '+%Y-%m-%d %H:%M:%S')

    echo "  pages_to_write: $TOTAL_PAGES, finish_calls: $FINISH_COUNT, latency: ${LATENCY} micros/op, throughput: ${THROUGHPUT} MB/s"
    echo "overwrite,$NUM_OPS,true,$gc_level,$FINISH_THRESHOLD,$TOTAL_PAGES,$FINISH_COUNT,$LATENCY,$THROUGHPUT,$TIMESTAMP" >> $CSV_FILE
done
